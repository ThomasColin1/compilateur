...
for jobname in jobs:
    ...
    gccstatus=command("gcc -Wall -Wpedantic -std=c18 -S -o asm-gcc.s input.c", "gcc-compile.txt")
    if gccstatus == 0:
        # test-case is a valid program. we should run it
        gccstatus=command("gcc -Wall -Wpedantic -std=c18 -o exe-gcc asm-gcc.s", "gcc-link.txt")
